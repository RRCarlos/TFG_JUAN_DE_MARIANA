# CONTEXTO DEL PROYECTO - TFG JUAN DE MARIANA

## Para uso de la IA

---

## 1. PROYECTO

**Título**: Análisis Historiográfico de las Fuentes de la Historia General de España (Libros I-IV)

**Pregunta de investigación**: ¿Realmente tuvo Juan de Mariana acceso a las obras que cita en su Historia General de España?

**Estudiante**: RRCarlos  
**Repositorio**: https://github.com/RRCarlos/TFG_Juan_de_Mariana.git

---

## 2. ESTADO ACTUAL

| Fase | Descripción | Estado |
|------|-------------|--------|
| 1-2 | Lectura y extracción de autores | ✅ Completado |
| 3 | Verificación en catálogos | ✅ Completado |
| 4-6 | Autores adicionales detectados | ✅ Completado |
| 7 | Verificación en texto original | ✅ Completado |
| 8 | Verificación ediciones antes de 1592 | ✅ Completado |
| 8B | Tradición textual | ✅ Completado |
| **9** | **Biblioteca Regional CLM** (catálogo online) | ✅ Completado |
| **9B** | **Verificación presencial en biblioteca** | ⏳ Pendiente (requiere visita) |
| **9C** | **Verificación cruzada de resultados** | ✅ Completado |
| **10** | Exlibris jesuitas | ⏳ Pendiente |
| **11** | Análisis y conclusiones | ⏳ Pendiente |

---

## 3. REGLAS ABSOLUTAS

### Formato de citas (OBLIGATORIO)
```
[Autor]. [Título]. [Año]. Disponible en: [URL]
```

### URLs válidas
- BNE: https://www.bne.es
- BVMC: https://bvpb.mcu.es
- Gallica: https://gallica.bnf.fr
- Internet Archive: https://archive.org
- VIAF: https://viaf.org
- USTC: https://ustc.ac.uk

### Criterios de verificación

| Fase | Qué cuenta | Qué NO cuenta |
|------|------------|----------------|
| 8 | ✅ Incunables (hasta 1500), siglo XVI (hasta 1592) | ❌ Manuscritos medievales, ediciones posteriores a 1592 |
| 9 | ✅ Ejemplar en Biblioteca Regional CLM | ❌ Solo consulta de catálogo |
| 10 | ✅ Exlibris jesuitas (sello, manuscrito, superlibro, signatura antigua) | ❌ Sellos modernos de la biblioteca |

---

## 4. AUTORES PROBLEMÁTICOS (9)

| # | Autor | Problema |
|---|-------|----------|
| 33 | Falso Beroso (Annio de Viterbo) | Falsificación del siglo XV |
| 34 | Filisto de Siracusa | Obra perdida, solo fragmentos |
| 35 | Quinto Fabio Píctor | Obra perdida |
| 41 | El moro Rasis (al-Razi) | Principalmente manuscrito |
| 48 | Miguel Sincelo | Principalmente manuscrito |
| 51 | Braulio de Zaragoza | Sin edición incunable |
| 57 | San Ildefonso de Toledo | Sin edición incunable |
| 63 | Filón de Biblos | Solo fragmentos |
| 68 | Andrea de' Bardi (El Anconitano) | Autor dudoso |

---

## 5. RESULTADOS CLAVE

- **68 autores únicos** verificados
- **85.3%** (58/68) tienen ediciones impresas anteriores a 1592
- **Tradición textual**: ~70% impresas, ~20% compilaciones, ~10% manuscritos

---

## 6. UBICACIÓN DE ARCHIVOS

Documentos finales en: `C:\Users\PC\TFG_MARIANA_CONSOLIDADO\`

- CONTEXTO_OPENCODE.md (este archivo)
- MEMORIA.md
- AUTORES.md

---

*Documento actualizado: 14 de abril de 2026*